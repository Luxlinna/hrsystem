 ...
 */
export function getOpsBuLogo(): string {
 ...
 */
export function getBuCompanyName(fallback?: string): string {
  ...
  return fallback || DEFAULT_BU_COMPANY_NAME;
}
...
export function resolveDocumentBranding(params: {
  ...
  return {
    logo: getOpsBuLogo(),
    companyName: params.businessUnit || getBuCompanyName(), // "OPS Solutions Co., Ltd."
    companyKhmer: "",
    isHrDivision: false,
  };
}
 ...
 */
export function isCustomOpsLogoActive(): boolean {
  const current = getOpsBuLogo();
 ...
 */
export async function saveOpsBuLogo(logoBase64?: string | null, buCompanyName?: string | null): Promise<void> {
  const now = new Date().toISOString();
  const upserts: Array<{ key: string; value: string; updated_at: string }> = [];
 ...
 */
export async function resetOpsBuLogo(): Promise<void> {
  await saveOpsBuLogo("", DEFAULT_BU_COMPANY_NAME);
}

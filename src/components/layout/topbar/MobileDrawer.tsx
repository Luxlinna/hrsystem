...
interface MobileDrawerProps {
  open: boolean; onClose: () => void; displayName: string;
  avatarUrl?: string; userEmail?: string; handleLogout: () => void;
  can: (module: string) => boolean; isAdmin: boolean;
  isBranchAdmin?: boolean; canOpenRecycleBin: boolean;
}
...
const MobileDrawer = memo(function MobileDrawer({
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    touchStartX.current = e.touches[0].clientX; touchCurrentX.current = e.touches[0].clientX;
    isDragging.current = true; setDragX(0);
  };
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
          ...
          <div className="flex items-center gap-1">
            ...
            >
              {isDark ? <i className="ri-sun-line text-lg text-amber-400" /> : <i className="ri-moon-line text-lg text-gray-300" />}
            </button>
            <button
              onClick={onClose}
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      to={item.path}
                      onClick={onClose}
                      className={`flex items-center gap-3 mx-3 px-3 py-2.5 rounded-lg text-[13px] transition-colors ${isActive ? "bg-[#29ABE2]/20 text-[#29ABE2]" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
                    >

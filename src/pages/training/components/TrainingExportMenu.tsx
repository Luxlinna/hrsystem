...
import {
  ...
  exportEnrollmentsXLSX,
  exportEnrollmentsCSV,
  exportCertificatesPDF,
  exportCertificatesXLSX,
  exportCertificatesCSV,
} from "../exportUtils";

interface TrainingExportMenuProps {
...
export const TrainingExportMenu = memo(function TrainingExportMenu({
  activeTab,
  courses,
  enrollments,
  certificates,
  disabled = false,
}: TrainingExportMenuProps) {
    ...
    async (fmt: Format) => {
      ...
      try {
        if (activeTab === "certificates") {
          if (fmt === "pdf") exportCertificatesPDF(certificates);
          else if (fmt === "xlsx") await exportCertificatesXLSX(certificates);
          else if (fmt === "csv") exportCertificatesCSV(certificates);
        } else if (activeTab === "enrollments") {
          if (fmt === "pdf") exportEnrollmentsPDF(enrollments);
          else if (fmt === "xlsx") await exportEnrollmentsXLSX(enrollments);
          else if (fmt === "csv") exportEnrollmentsCSV(enrollments);
        } else {
          // Courses catalog
          if (fmt === "pdf") exportCoursesPDF(courses);
          else if (fmt === "xlsx") await exportCoursesXLSX(courses);
          else if (fmt === "csv") exportCoursesCSV(courses);
        }
      } finally {
        setTimeout(() => setExporting(null), 700);
      }
    },
    [activeTab, courses, enrollments, certificates]
  );
  ...
  const getScopeLabel = () => {
    if (activeTab === "certificates") return "Certificates Register";
    if (activeTab === "enrollments") return "Staff Enrollments";
  ...
  const exportOptions = [
    {
      ...
      ext: ".pdf",
      desc: "Print-ready summary report with metrics & roster",
    ...
    },
    {
      ...
      ext: ".xlsx",
      desc: "Multi-column structured training spreadsheet",
    ...
    },
    {
      ...
      ext: ".csv",
      desc: "Raw comma-separated training records file",

...
const TABLES_TO_SUBSCRIBE = [
  ...
  "payroll_records", "onboarding_requests", "branches", "notifications",
...
export function useCompanyDashboardData({
  ...
  const loadAllData = useCallback(async () => {
    if (isPartnerBranchBlocked || !targetBranch) {
      setEmployees([]); setOnboarding([]); setLeaveRequests([]); setPayroll([]);
    ...
    const [
      { data: b }, { data: e }, { data: ob }, { data: lr }, { data: pr },
      ...
      supabase.from("payroll_records").select("id, employee_id, month, net_pay, status, employees(branch_id)").is("deleted_at", null).gte("month", fromMonth).lte("month", toMonth),
    ...
    setStats({
      ...
      payrollProcessed: filteredPr.filter((p: any) => p.status === "processed").length,
      payrollTotal: filteredPr.reduce((sum: number, p: any) => sum + Number(p.net_pay || 0), 0),
  ...
  useEffect(() => {
    ...
    const channels = TABLES_TO_SUBSCRIBE.map((table) =>
      supabase.channel(`dash-${table}`).on("postgres_changes", { event: "*", schema: "public", table }, debouncedLoad).subscribe()
    );
  ...
  return {
    stats, employees, onboarding, leaveRequests, payroll, notifications,
    jobs, candidates, announcements, loading, refreshing, deptData,
    lastUpdated, hrKpis, attendanceData, hiringTrend, handleRefresh,
  };
}

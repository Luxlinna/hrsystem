...
export function useCompanyDashboard() {
  ...
  const loadAllData = useCallback(async () => {
    ...
    const [
      ...
      supabase.from("leave_requests").select("id, status, leave_type, start_date, end_date, created_at, employees(first_name, last_name, role, department)").order("created_at", { ascending: false }).limit(5),
    ...
    setHrKpis(computeHrKpis(attData || [], trainEnroll || [], discData || []));
    setAttendanceData(computeAttendanceBreakdown(attData || []));

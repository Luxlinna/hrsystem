...
import {
  exportAnnouncementsPDF,
  exportAnnouncementsXLSX,
  exportAnnouncementsCSV,
} from "../exportUtils";

interface AnnouncementExportMenuProps {
...
export const AnnouncementExportMenu = memo(function AnnouncementExportMenu({
  announcements,
  disabled = false,
}: AnnouncementExportMenuProps) {
  ...
  const handleExport = useCallback(
    async (fmt: Format) => {
      ...
      setOpen(false);
      try {
        if (fmt === "pdf") exportAnnouncementsPDF(announcements);
        else if (fmt === "xlsx") await exportAnnouncementsXLSX(announcements);
        else if (fmt === "csv") exportAnnouncementsCSV(announcements);
      } finally {
  ...
  const exportOptions = [
    {
      fmt: "pdf" as Format,
      label: "PDF Broadcast Report",
      ext: ".pdf",
      desc: "Print-ready summary report with KPI stats and articles",
      icon: "ri-file-pdf-line",
      color: "text-rose-600 bg-rose-50 group-hover:bg-rose-100",
    },
    {
      fmt: "xlsx" as Format,
      label: "Excel Announcements Feed",
      ext: ".xlsx",
      desc: "Multi-column structured news and bulletin workbook",
    ...
    },
    {
      ...
      label: "CSV Feed Data",
      ext: ".csv",
      desc: "Raw comma-separated broadcasts and audience data",

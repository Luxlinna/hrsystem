...
export function exportItemizedRecordsPDF(records: EmployeeItemRecord[], title = "Itemized Employee Payslips Audit"): boolean {
  ...
  const rows = records.length > 0
    ? records
        ...
        .join("")
    : `<tr><td colspan="7" style="text-align:center;padding:24px;color:#64748b;">No itemized payroll records found.</td></tr>`;
  ...
  <body>
    <div class="header-box">
      <div>
        <h1>HRM_OPS — ${title}</h1>
        <div class="meta">Governance &middot; Itemized Line-Item Audit</div>
      </div>
      <div class="meta" style="text-align:right">
        <div><strong>Generated:</strong> ${new Date().toLocaleString("en-US")}</div>
        <div><strong>Headcount:</strong> ${total} Payslips</div>
      </div>
    </div>

    <div class="stats-grid">
      <div class="stat-card"><div class="stat-val">$${totalBase.toLocaleString()}</div><div class="stat-lbl">Total Base</div></div>
      ...
      <div class="stat-card"><div class="stat-val" style="color:#253C7D">$${totalNet.toLocaleString()}</div><div class="stat-lbl">Total Net Outlay</div></div>
    </div>
    ...
    <div class="footer">
      <div>HRM_OPS Enterprise HRMS &middot; Itemized Payslip Verification</div>
      <div>Page 1 of 1</div>
    </div>
  </body>
  </html>`;

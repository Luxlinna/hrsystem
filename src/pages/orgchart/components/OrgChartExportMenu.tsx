...
interface OrgChartExportMenuProps {
  employees: Employee[];
  branchName?: string;
  disabled?: boolean;
}
...
export const OrgChartExportMenu = memo(function OrgChartExportMenu({
  employees,
  branchName,
  disabled = false,
}: OrgChartExportMenuProps) {
  ...
  const handleExport = useCallback(
    async (fmt: Format) => {
      ...
      try {
        if (fmt === "pdf") {
          exportOrgChartPDF(employees, branchName);
        } else if (fmt === "xlsx") {
          await exportOrgChartXLSX(employees, branchName);
        } else if (fmt === "csv") {
          exportOrgChartCSV(employees, branchName);
        }
      } finally {
  ...
  const exportOptions = [
    {
      fmt: "pdf" as Format,
      label: "PDF Org Hierarchy Report",
      ext: ".pdf",
      desc: "Print-ready leadership & reporting chains",
      icon: "ri-file-pdf-line",
      color: "text-rose-600 bg-rose-50 group-hover:bg-rose-100",
    },
    {
      fmt: "xlsx" as Format,
      label: "Excel Structure Workbook",
      ext: ".xlsx",
      desc: "Multi-column employee hierarchy spreadsheet",
      icon: "ri-file-excel-2-line",
      color: "text-emerald-600 bg-emerald-50 group-hover:bg-emerald-100",
    },
    {
      fmt: "csv" as Format,
      label: "CSV Directory Data",
      ext: ".csv",
      desc: "Raw comma-separated reporting data",

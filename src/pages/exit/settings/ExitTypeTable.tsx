...
import type { ExitTypeSetting } from "./types";

interface ExitTypeTableProps {
  items: ExitTypeSetting[];
  loading: boolean;
  onEdit: (item: ExitTypeSetting) => void;
  onToggleStatus: (item: ExitTypeSetting) => void;
  onDelete: (id: string) => void;
}

export const ExitTypeTable = memo(function ExitTypeTable({
  items,
  loading,
  onEdit,
  onToggleStatus,
  ...
  if (loading) {
    return (
      <div className="py-16 text-center text-gray-400 dark:text-slate-500">
        ...
        <span className="text-xs font-medium">Loading exit types...</span>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-xs font-bold text-gray-700 dark:text-slate-200">No exit types found</p>
        <p className="text-[11px] text-gray-400 mt-0.5">Click + Add Exit Type to create one</p>
      </div>
    );
  }

...
export function useAnalyticsData() {
  ...
  const loadData = useCallback(async () => {
    ...
    setJobs(rawJobs);
    setCandidates((results[4].data || []).filter((c: any) => !c.job_id || jobIds.has(c.job_id)));
    ...
    setItAssets((results[7].data || []).filter((a: any) => !a.assigned_to || empIds.has(a.assigned_to) || a.employees?.branch_id === targetBranch));

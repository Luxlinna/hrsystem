...
export const NewPasswordForm = memo(function NewPasswordForm({
  ...
  const isPhone = isPhoneSyntheticEmail(invitedEmail);
  const displayContact = isPhone
    ? formatPhoneDisplay(syntheticEmailToPhone(invitedEmail))
    : invitedEmail;
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label className="block text-[12px] font-semibold text-gray-700 mb-1.5">
          {isPhone ? "Invited Phone Number" : "Invited Email"}
        </label>
        <div className="relative">
          <i
            className={`${
              isPhone ? "ri-phone-line" : "ri-mail-line"
            } absolute left-3.5 top-1/2 -translate-y-1/2 text-base text-gray-400 pointer-events-none`}
          />
          <input
            type={isPhone ? "text" : "email"}
            value={displayContact}
            readOnly
            aria-label={isPhone ? "Invited phone number" : "Invited email"}
            className="w-full cursor-not-allowed bg-gray-50 pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 text-[13px] text-gray-700 font-semibold font-mono"
            placeholder={isPhone ? "Loading phone number..." : "Loading invited email..."}
          />
        </div>
      </div>

...
import {
  ...
  exportBenefitEnrollmentsPDF,
  exportBenefitEnrollmentsXLSX,
  exportBenefitEnrollmentsCSV,
} from "../exportUtils";
...
export const BenefitsExportMenu = memo(function BenefitsExportMenu({
  tab,
  plans,
  enrollments,
  disabled = false,
}: BenefitsExportMenuProps) {
  const [open, setOpen] = useState(false);
  const [exporting, setExporting] = useState<Format | null>(null);
  const menuRef = useRef<HTMLDivElement>(null);

  const isEnrollmentTab = tab === "enrollment";
  const recordCount = isEnrollmentTab ? enrollments.length : plans.length;
  const scopeLabel = isEnrollmentTab ? "Benefit Enrollments" : "Benefit Programs";
  ...
  const handleExport = useCallback(
    async (fmt: Format) => {
      ...
      try {
        if (isEnrollmentTab) {
          if (fmt === "pdf") exportBenefitEnrollmentsPDF(enrollments);
          else if (fmt === "xlsx") await exportBenefitEnrollmentsXLSX(enrollments);
          else if (fmt === "csv") exportBenefitEnrollmentsCSV(enrollments);
        } else {
          if (fmt === "pdf") exportBenefitPlansPDF(plans, enrollments);
          else if (fmt === "xlsx") await exportBenefitPlansXLSX(plans, enrollments);
          else if (fmt === "csv") exportBenefitPlansCSV(plans, enrollments);
        }
      } finally {
        setTimeout(() => setExporting(null), 700);
      }
    },
    [isEnrollmentTab, enrollments, plans]
  );
  ...
  const exportOptions = [
    {
      ...
      ext: ".pdf",
      desc: "Print-ready summary report with coverage & metrics",
    ...
    },
    {
      ...
      ext: ".xlsx",
      desc: "Multi-column structured program & employee spreadsheet",
      icon: "ri-file-excel-2-line",
      color: "text-emerald-600 bg-emerald-50 group-hover:bg-emerald-100",
    },
    {
      fmt: "csv" as Format,
      label: `CSV ${scopeLabel} File`,
      ext: ".csv",
      desc: "Raw comma-separated insurance plan/enrollment dataset",
        ...
        <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-gray-100 rounded-2xl shadow-xl p-2 z-50 animate-in fade-in zoom-in-95 duration-100">
          <div className="px-3 py-1.5 border-b border-gray-100 mb-1 flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-gray-400">
              {recordCount} {isEnrollmentTab ? "records" : "plans"}
            </span>
          </div>

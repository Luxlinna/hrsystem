...
import { exportCalendarPDF, exportCalendarXLSX, exportCalendarCSV } from "../exportUtils";
...
export const CalendarExportMenu = memo(function CalendarExportMenu({
  ...
  const handleExport = useCallback(
    async (fmt: Format) => {
      ...
      try {
        if (fmt === "pdf") {
          exportCalendarPDF(filteredLeaves, month, year);
        } else if (fmt === "xlsx") {
          await exportCalendarXLSX(filteredLeaves, month, year);
        } else if (fmt === "csv") {
  ...
  const exportOptions = [
    {
      ...
      ext: ".pdf",
      desc: "Print-ready monthly team availability",
    ...
    },
    {
      ...
      ext: ".xlsx",
      desc: "Structured monthly absence workbook",
    ...
    },
    {
      ...
      label: "CSV Schedule",
      ext: ".csv",
      desc: "Raw comma-separated schedule data",
  ...
  return (
    <div className="relative inline-block" ref={menuRef}>
      <button
        ...
        disabled={disabled || filteredLeaves.length === 0}

...
import type { TaskForm, StaffMember, ChecklistTask } from "../../types";
import { CATEGORY_META, PRIORITY_META } from "../../constants";

interface EditChecklistTaskModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const EditChecklistTaskModal = memo(function EditChecklistTaskModal({
  isOpen,
  onClose,
  selectedTask,
  taskForm,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-3xl p-6 sm:p-8 shadow-2xl animate-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="text-base font-extrabold text-gray-900">Edit Checklist Task</h3>
            <p className="text-xs text-gray-400 mt-0.5">Modify task description, deadline, or assignment</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="text"
                list="staff-assignees-edit"
                value={taskForm.assigned_to}
                onChange={(e) => setTaskForm({ ...taskForm, assigned_to: e.target.value })}
                placeholder="Select or enter person..."
                className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs text-gray-900 font-medium focus:bg-white focus:outline-none focus:border-[#253C7D]"
              />
              <datalist id="staff-assignees-edit">
                {staff.map((s) => (

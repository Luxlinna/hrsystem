  ...
  return (
    <div
      className={`bg-white border border-gray-100 rounded-xl p-4 shadow-2xs ${
        !isEmployeeScoped ? "opacity-40 pointer-events-none" : ""
      }`}
    >
      <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
        Employee Filters{" "}
        {!isEmployeeScoped && <span className="normal-case font-normal">(not used by this report)</span>}
      </p>
      <div className="space-y-3">
        <div className={!isNameScoped ? "opacity-40 pointer-events-none" : ""}>
          <label className="text-xs text-gray-500 mb-1 block">
            Name{" "}
            {!isNameScoped && isEmployeeScoped && (
              <span className="normal-case font-normal">(not used by this report)</span>
            )}
          </label>
          <input
            type="text"

...
interface DocumentsExportMenuProps {
  documents: Document[];
  disabled?: boolean;
}
...
export const DocumentsExportMenu = memo(function DocumentsExportMenu({
  documents,
  disabled = false,
}: DocumentsExportMenuProps) {
  ...
  const handleExport = useCallback(
    async (fmt: Format) => {
      ...
      setOpen(false);
      try {
        if (fmt === "pdf") exportDocumentsPDF(documents);
        else if (fmt === "xlsx") await exportDocumentsXLSX(documents);
        else if (fmt === "csv") exportDocumentsCSV(documents);
      } finally {
  ...
  const exportOptions = [
    {
      fmt: "pdf" as Format,
      label: "PDF Catalog Report",
      ext: ".pdf",
      desc: "Print-ready summary report with metrics and files list",
      icon: "ri-file-pdf-line",
      color: "text-rose-600 bg-rose-50 group-hover:bg-rose-100",
    },
    {
      fmt: "xlsx" as Format,
      label: "Excel Document Index",
      ext: ".xlsx",
      desc: "Multi-column structured files and versions spreadsheet",
      icon: "ri-file-excel-2-line",
      color: "text-emerald-600 bg-emerald-50 group-hover:bg-emerald-100",
    },
    {
      fmt: "csv" as Format,
      label: "CSV Files Data",
      ext: ".csv",
      desc: "Raw comma-separated document repository dataset",

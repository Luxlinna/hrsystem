...
import { exportLeavePDF, exportLeaveXLSX, exportLeaveCSV } from "../exportUtils";

interface LeaveExportMenuProps {
...
export const LeaveExportMenu = memo(function LeaveExportMenu({
  filteredRequests,
  disabled = false,
  onToast,
}: LeaveExportMenuProps) {
  ...
  const handleExport = useCallback(
    async (fmt: Format) => {
      ...
      try {
        let success = false;
        if (fmt === "pdf") {
          success = exportLeavePDF(filteredRequests);
        } else if (fmt === "xlsx") {
          success = await exportLeaveXLSX(filteredRequests);
        } else if (fmt === "csv") {
          success = exportLeaveCSV(filteredRequests);
        }

        if (onToast) {
          onToast(
            success
              ? { type: "success", message: `Exported leave records to ${fmt.toUpperCase()}` }
  ...
  const exportOptions = [
    {
      ...
      label: "PDF Report",
      ext: ".pdf",
      desc: "Print-ready summary & status audit",
  ...
  return (
    <div className="relative inline-block" ref={menuRef}>
      <button
        ...
        disabled={disabled || filteredRequests.length === 0}
      ...
      {open && filteredRequests.length > 0 && (

...
export function exportLeavePDF(filteredRequests: LeaveRequest[], title = "Workforce Leave & Absence Report"): boolean {
  ...
  const totalDays = filteredRequests.reduce((acc, r) => acc + (Number(r.days) || 0), 0);
  ...
  <body>
    <div class="header-box">
      <div>
        <h1>HRM_OPS — ${title}</h1>
        <div class="meta">Official Absence & Leave Management Record</div>
      </div>
      <div class="meta" style="text-align:right">
        <div><strong>Generated:</strong> ${new Date().toLocaleString("en-US")}</div>
        <div><strong>Total Leave Taken:</strong> ${totalDays} Days</div>
      </div>
    </div>
    ...
    <div class="footer">
      <div>HRM_OPS Enterprise HRMS &middot; Confidential Leave Record</div>
      <div>Page 1 of 1</div>
    </div>
  </body>
  </html>`;
